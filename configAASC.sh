...
az spring create -n $SPRING_CLOUD_SERVICE -g $RESOURCE_GROUP -l $REGION --disable-app-insights false
# --subscription $SUBSCRIPTION

# az spring config-server set --config-file $CONFIG_DIR/application.yml -n $SPRING_CLOUD_SERVICE -g $RESOURCE_GROUP
# OR
az spring config-server git set -n ${SPRING_CLOUD_SERVICE} -g $RESOURCE_GROUP --uri $REPO_OWNER_URI/$CONFIG_REPO


# Create app constructs in ASC
printf "\n\nCreating the apps in Spring Cloud\n"

az spring app create -n $API_GATEWAY_ID -g $RESOURCE_GROUP -s $SPRING_CLOUD_SERVICE \
    --instance-count 1 --memory 2Gi --runtime-version Java_17 --assign-endpoint true
az spring app create -n $ADMIN_SERVICE_ID -g $RESOURCE_GROUP -s $SPRING_CLOUD_SERVICE \
    --instance-count 1 --memory 2Gi --runtime-version Java_17 --assign-endpoint true
az spring app create -n $AIRPORT_SERVICE_ID -g $RESOURCE_GROUP -s $SPRING_CLOUD_SERVICE \
    --instance-count 1 --memory 2Gi --runtime-version Java_17 --assign-endpoint true
az spring app create -n $WEATHER_SERVICE_ID -g $RESOURCE_GROUP -s $SPRING_CLOUD_SERVICE \
    --instance-count 1 --memory 2Gi --runtime-version Java_17 --assign-endpoint true
az spring app create -n $CONDITIONS_SERVICE_ID -g $RESOURCE_GROUP -s $SPRING_CLOUD_SERVICE \
    --instance-count 1 --memory 2Gi --runtime-version Java_17 --assign-endpoint true
...
WEBAPP_RESOURCE_ID=$(az spring show -n $SPRING_CLOUD_SERVICE -g $RESOURCE_GROUP --query id --output tsv)
